...
sub <- subset(cd,Date == c("1/2/2007", '2/2/2007') )
dt <- strptime(paste(sub$Date, sub$Time, sep = ' '), '%d/%m/%Y %H:%M:%S' )

par(mfrow = c(2, 2)) 
plot(dt,sub$Global_active_power,type="l", 
     xlab="Date",ylab="Global Active Power")
plot(dt, sub$Voltage, type = 'l', xlab = 'Day', ylab = 'Voltage')
...
legend('topright', lty = c(1,1,1), col=clr, legend = names, cex = .5)
plot(dt, sub$Global_reactive_power, type = 'l', xlab = 'Day', ylab = 'Global Reactive Power')
